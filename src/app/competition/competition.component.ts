...
export class CompetitionComponent implements OnInit {
  ...
  currClassements: Array<Classement> = []
  ...
  ngOnInit() {
    this.activatedRoute.paramMap.pipe(
      ...
      switchMap((id: string) => forkJoin({
        ...
        scorers: this.dataService.getScorerFrom(id)
      }))
    ).subscribe({
      next: ({ competition, matches, scorers }) => {
        this.competition = competition
        this.matches = matches
        this.stage = this.setStages(this.matches)
        this.matchday = this.setMatchdays(this.matches)
        this.addBracket()
        this.competition.classements?.push(scorers)
        ...
        this.currClassements = this.competition.classements!.filter(classement => classement.stage == this.currStage || classement.stage == 'SCORERS')
      },
  ...
  addBracket() {
    this.stage.forEach((stage: string) => {
      ...
      if (stage != 'GROUP_STAGE' && stage != 'REGULAR_SEASON') {
        ...
        this.competition.classements?.push(...brackets)
      }
    })
  }

  sortBracket(b_: Array<Classement>, last?: Classement) {
    if (last) {
  ...
  label(classement: Classement): string {
    switch (classement.stage) {
      case 'REGULAR_SEASON':
        switch (classement.type) {
          case 'TOTAL':
            return 'Global'
          case 'HOME':
            return 'Domicile'
        ...
        break;
      case 'GROUP_STAGE':
        return 'Groupe ' + classement.group.charAt(classement.group.length - 1)
  ...
  changeStage(stage: string) {
    ...
    this.currClassements = this.competition.classements!.filter((classement: Classement) => classement.stage == this.currStage || classement.stage == 'SCORERS')
  }
